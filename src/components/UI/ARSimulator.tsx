...
export function ARSimulator() {
      ...
      <div className={`fixed inset-0 z-40 flex items-center justify-center p-4 overflow-y-auto uppercase ${isDark ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-800'}`}>
        ...
        <div className={`w-full max-w-[440px] relative z-10 p-6 rounded-3xl border backdrop-blur-xl shadow-2xl space-y-6 font-mono text-[9px] ${
          ...
          <div className="space-y-3">
            ...
            {!isMobileDevice && (
              <div className="p-3 bg-amber-500/10 border border-amber-500/20 text-amber-500 dark:text-amber-400 rounded-xl text-[8px] leading-relaxed font-sans normal-case">
                ⚠️ desktop preview mode active. spatial room tracking and gyro tilt flight controls are only supported on mobile devices. proceed using keyboard keys.
              </div>
            )}
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black overflow-hidden flex flex-col justify-between">
      ...
      {/* 1. BACKGROUND LAYER: Webcam passthrough for fallback preview mode or premium virtual lab backdrop */}
      {!isPresenting && (
        <div className="absolute inset-0 z-0 bg-slate-950">
          {cameraStream ? (
            ...
            <div 
              ...
              style={{ backgroundImage: `url('/ar_fallback_bg.png')` }}
            />
          )}
        </div>
      )}
