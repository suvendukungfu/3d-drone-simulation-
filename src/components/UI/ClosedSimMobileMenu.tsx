...
function MenuSection({
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-slate-200 bg-white shadow-sm">
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-full flex items-center justify-between px-4 py-3.5 bg-slate-50/80 hover:bg-slate-100 transition-colors active:bg-slate-200/50 touch-manipulation"
      >
...
function TelRow({
  ...
  return (
    <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-slate-50/50 border border-slate-200/60 hover:border-slate-350 hover:bg-slate-50 transition-all">
      ...
      <div className="flex items-baseline gap-1">
        ...
        {unit && (
          <span className="text-[9px] text-slate-450 font-mono font-medium">{unit}</span>
        )}
      </div>
    </div>
  );
...
function ToggleRow({
  ...
  return (
    <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-slate-50/50 border border-slate-200/60">
      <span className="text-[11px] text-slate-700 font-semibold">{label}</span>
      <button
        onClick={onToggle}
        className={`relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-all duration-300 touch-manipulation outline-none ${
          active ? 'bg-gradient-to-r from-blue-600 to-indigo-600 shadow-[0_0_8px_rgba(79,70,229,0.25)]' : 'bg-slate-200'
        }`}
      >
        <span
  ...
  return (
    <button
      ...
      className={`flex-1 py-2 rounded-lg text-[10px] font-bold uppercase tracking-widest transition-all duration-300 touch-manipulation border relative overflow-hidden ${
        active
          ? 'bg-gradient-to-r from-blue-600 to-indigo-650 border-indigo-600 text-white shadow-[0_2px_8px_rgba(79,70,229,0.25)] font-extrabold'
          : 'bg-white border-slate-200 text-slate-500 hover:text-slate-800 hover:bg-slate-50 hover:border-slate-350'
      }`}
    >
      {active && (
        <span className="absolute inset-0 bg-gradient-to-r from-white/10 via-transparent to-transparent animate-pulse" />
      )}
      {label}
    </button>
  );
...
function ActionBtn({
  ...
  const colors = {
    default:
      'bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100 hover:border-slate-350 active:bg-slate-200/50 shadow-sm',
    danger:
      'bg-red-50 border-red-200 text-red-700 hover:bg-red-100 hover:border-red-300 active:bg-red-200/50 shadow-[0_2px_6px_rgba(239,68,68,0.06)]',
    success:
      'bg-emerald-50 border-emerald-200 text-emerald-700 hover:bg-emerald-100 hover:border-emerald-300 active:bg-emerald-200/50 shadow-[0_2px_6px_rgba(16,185,129,0.06)]',
    warning:
      'bg-amber-50 border-amber-200 text-amber-700 hover:bg-amber-100 hover:border-amber-300 active:bg-amber-200/50 shadow-[0_2px_6px_rgba(245,158,11,0.06)]',
  };
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`flex items-center gap-3 w-full px-4 py-3 rounded-xl border text-[11px] font-extrabold uppercase tracking-wider transition-all duration-200 active:scale-[0.98] touch-manipulation ${colors[variant]} ${
...
function PidSlider({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="flex justify-between text-[9px] font-mono text-slate-500 uppercase tracking-widest font-bold">
        <span>{label}</span>
        <span className="text-indigo-600 font-extrabold">{val.toFixed(3)}</span>
      </div>
      <input
        type="range"
        min={min}
...
export function ClosedSimMobileMenu({
  ...
  const getBattColor = (pct: number) =>
    pct < 20 ? 'text-red-600 font-bold' : pct < 40 ? 'text-amber-600 font-bold' : 'text-emerald-600 font-bold';
  ...
  return (
    <>
      ...
      <div className="pluto-mobile-overlay select-none">
        ...
        <div id="tutorial-left-joystick" className="pluto-joystick-container pluto-joystick-left-container left-[max(24px,env(safe-area-inset-left))] z-30">
          <PlutoJoystick
            side="left"
        ...
        <div id="tutorial-right-joystick" className="pluto-joystick-container pluto-joystick-right-container right-[max(24px,env(safe-area-inset-right))] z-30">
          <PlutoJoystick
            side="right"
        ...
        <div className="pluto-center-actions-container z-25">
        ...
        <div className={`pluto-sidebar-container absolute right-[max(16px,env(safe-area-inset-right))] top-[max(64px,calc(56px+env(safe-area-inset-top,0px)))] flex flex-col landscape:flex-row gap-2.5 pluto-sidebar-transition ${
        ...
        <div className={`pluto-firmware-label absolute bottom-5 right-[max(180px,env(safe-area-inset-right))] text-[8px] font-mono text-white tracking-widest uppercase select-none text-right pluto-sidebar-transition ${
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="block fixed inset-0 z-[65] bg-black/45 backdrop-blur-[2px]"
              onClick={close}
            />
            ...
            <motion.div
              ...
              className="flex fixed top-0 left-0 bottom-0 z-[70] w-[85vw] max-w-[320px] min-w-[260px] flex-col bg-white border-r border-slate-200 shadow-[8px_0_32px_rgba(0,0,0,0.06)]"
              style={{ paddingBottom: 'env(safe-area-inset-bottom)', paddingTop: 'env(safe-area-inset-top, 0px)' }}
            >
              {/* Header */}
              <div className="flex items-center justify-between px-4 pt-5 pb-3.5 border-b border-slate-100 shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[10px] text-indigo-600 font-extrabold uppercase tracking-widest leading-none">
                      Pluto Controller
                    </p>
                    <p className="text-[13px] font-extrabold text-slate-800 leading-tight capitalize tracking-wide mt-1">
                ...
                <button
                  onClick={close}
                  className="w-8 h-8 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-500 hover:text-slate-850 hover:bg-slate-200 transition-all duration-200 active:scale-90 touch-manipulation"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-4 pt-4 pb-6 space-y-3.5 scrollbar-thin">
                ...
                <div className="pb-3 border-b border-slate-100">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border border-indigo-200 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-[11px] font-extrabold uppercase tracking-widest transition-all duration-300 active:scale-[0.97]"
                  >
                ...
                <div>
                  <div className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border text-[10px] font-bold uppercase tracking-widest transition-all duration-300 ${
                    telemetry?.isArmed
                      ? 'bg-red-50 border-red-200 text-red-700 shadow-[0_2px_8px_rgba(239,68,68,0.06)]'
                    ...
                    <span className="relative flex h-2 w-2">
                      ...
                      <span className={`relative inline-flex rounded-full h-2 w-2 ${
                        telemetry?.isArmed ? 'bg-red-600' : 'bg-slate-350'
                      }`} />
                    </span>
                    <span className="font-extrabold tracking-widest leading-none">
                ...
                >
                  <div className="grid grid-cols-2 gap-2">
                    <TelRow
                      ...
                      color={telemetry?.isArmed ? 'text-red-600 font-bold' : 'text-slate-400'}
                    />
                    <TelRow
                      label="Flight Mode"
                    ...
                    <TelRow
                      ...
                      color={telemetry ? getBattColor(telemetry.battery) : 'text-slate-800'}
                    />
                    <TelRow
                      label="Flight Time"
                  ...
                  <TelRow label="Signal" value="STRONG" color="text-emerald-600" />
                </MenuSection>
                    ...
                    {[
                      ...
                      const label =
                        status === 'ok'
                          ...
                          : 'ERROR';
                      return (
                        <div key={s.key} className="bg-slate-50/50 border border-slate-200/65 rounded-lg p-2 flex flex-col justify-between hover:border-slate-350 hover:bg-slate-50 transition-all">
                          <span className="text-[9px] font-mono text-slate-455 uppercase tracking-wider font-semibold">{s.name}</span>
                          <div className="flex items-center gap-1.5 mt-1">
                            <span className={`w-1.5 h-1.5 rounded-full ${dot}`} />
                            <span className={`text-[10px] font-mono font-bold ${
                              status === 'ok' ? 'text-emerald-600' : status === 'calibrating' ? 'text-amber-600' : 'text-red-600'
                            }`}>{label}</span>
                          </div>
                        </div>
                      );
                ...
                >
                  <div className="space-y-2">
                    <ActionBtn
                      icon={telemetry?.isArmed ? <ShieldOff className="w-3.5 h-3.5 text-red-500" /> : <Shield className="w-3.5 h-3.5 text-emerald-650" />}
                    ...
                    {telemetry?.isArmed && (
                      <>
                        <ActionBtn
                          icon={<Navigation className="w-3.5 h-3.5 rotate-45 text-blue-650" />}
                          label="Takeoff Drone"
                        ...
                        />
                        <ActionBtn
                          icon={<ChevronDown className="w-3.5 h-3.5 text-amber-650" />}
                        ...
                        <div className="lg:hidden w-full">
                          <ActionBtn
                            icon={<RefreshCw className="w-3.5 h-3.5 animate-spin-slow text-indigo-650" />}
                    ...
                    <ActionBtn
                      icon={<RefreshCw className="w-3.5 h-3.5 text-amber-650" />}
                      label="Reset Drone"
                ...
                >
                  <div className="space-y-3.5 p-1">
                    <PidSlider
                      ...
                      max={0.20}
                      step={0.005}
                      getValue={() => orchestrator?.controller.rollRateGains.kp ?? 0.06}
                      setValue={(v) => {
                        if (orchestrator) {
                          orchestrator.controller.rollRateGains.kp = v;
                          orchestrator.controller.pitchRateGains.kp = v;
                        }
                      }}
                    />
                    <PidSlider
                      ...
                      max={0.10}
                      step={0.005}
                      getValue={() => orchestrator?.controller.rollRateGains.ki ?? 0.03}
                      setValue={(v) => {
                        if (orchestrator) {
                          orchestrator.controller.rollRateGains.ki = v;
                          orchestrator.controller.pitchRateGains.ki = v;
                        }
                      }}
                    />
                    <PidSlider
                      ...
                      max={0.005}
                      step={0.0001}
                      getValue={() => orchestrator?.controller.rollRateGains.kd ?? 0.001}
                      setValue={(v) => {
                        if (orchestrator) {
                          orchestrator.controller.rollRateGains.kd = v;
                          orchestrator.controller.pitchRateGains.kd = v;
                        }
                      }}
                    />
                  </div>
                ...
                >
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <div key={label} className="bg-slate-50/50 border border-slate-200/60 rounded-lg p-2 flex flex-col justify-between">
                        <span className="text-[9px] font-mono text-slate-500 block font-semibold">{label}</span>
                        <div className="mt-1.5 h-1 bg-slate-200 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-gradient-to-r from-blue-500 to-indigo-650 rounded-full transition-all duration-75"
                            style={{ width: `${Math.abs(value) * 100}%`, marginLeft: value < 0 ? 'auto' : undefined }}
                          />
                        </div>
                        <span className="text-[10px] font-mono text-slate-600 mt-1 block font-semibold">
                          {(value * 100).toFixed(0)}%
                        </span>
                      </div>
                    ))}
                ...
                <div className="pt-4 pb-2 border-t border-slate-100">
                  <p className="text-[9px] text-slate-400 font-mono text-center uppercase tracking-widest font-semibold">
      ...
      {/* Post Landing Workflow Dialog disabled per user request */}
