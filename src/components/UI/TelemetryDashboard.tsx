    ...
    <div className="absolute inset-0 pointer-events-none z-10 font-sans select-none flex flex-col justify-between p-4">
      ...
      {notifications.length > 0 && (
        <div className="absolute top-16 left-1/2 -translate-x-1/2 z-30 pointer-events-none flex flex-col gap-2 items-center">
          {notifications.map((n) => {
            let bg = 'bg-white/95 border-slate-200 text-slate-800 shadow-md';
            let icon = <Info className="w-4 h-4 text-blue-500" />;
            if (n.type === 'success') {
              bg = 'bg-emerald-50 border-emerald-300 text-emerald-800 shadow-[0_8px_20px_rgba(16,185,129,0.06)]';
              icon = <CheckCircle className="w-4 h-4 text-emerald-600" />;
            } else if (n.type === 'warning') {
              bg = 'bg-amber-50 border-amber-300 text-amber-805 shadow-[0_8px_20px_rgba(245,158,11,0.06)]';
              icon = <AlertTriangle className="w-4 h-4 text-amber-600" />;
            } else if (n.type === 'error') {
              bg = 'bg-red-50 border-red-300 text-red-800 shadow-[0_8px_20px_rgba(239,68,68,0.06)]';
      ...
      {activeMissionIndex >= 0 && !isAcademyOpen && showChecklist && (
        <div className="absolute top-20 left-4 bg-white/95 border border-slate-200 p-4 rounded-xl w-64 shadow-[0_8px_25px_rgba(0,0,0,0.03)] pointer-events-auto z-20 flex flex-col gap-2.5">
            ...
            {MISSIONS[activeMissionIndex].objectives.map((obj, idx) => {
              ...
              return (
                <div key={idx} className="flex gap-2 items-start text-[10px] leading-relaxed">
                  ...
                  <span className={completed ? "text-emerald-700 font-semibold" : "text-slate-500"}>{obj}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="w-full flex justify-between items-start pointer-events-auto gap-4">
        ...
        {showTelemetryDashboard ? (
          <div className="flex flex-col gap-2">
            <div className="bg-white/95 backdrop-blur-md border border-slate-200 p-3.5 rounded-xl flex items-center gap-4 shadow-[0_8px_25px_rgba(0,0,0,0.02)]">
              <div className="w-9 h-9 rounded-lg bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-600">
                <Cpu className="w-5 h-5" />
              </div>
              <div>
                <h2 className="text-xs font-bold text-slate-900 uppercase tracking-wider">Avionics System</h2>
                {telemetry.calibrationActive ? (
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    <span className="text-[9px] font-mono text-red-650 font-bold">CALIBRATION FAILED</span>
                  </div>
                ) : (
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>
                    <span className="text-[9px] font-mono text-slate-500">ONLINE / CALIBRATED</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white/95 backdrop-blur-md border border-slate-200 p-2 rounded-xl flex gap-1 shadow-[0_8px_25px_rgba(0,0,0,0.02)] text-[9px] font-bold uppercase tracking-wider flex-wrap max-w-[320px]">
        ...
        {showTelemetryDashboard ? (
          <div className="flex flex-col items-center flex-1 max-w-[400px]">
            {/* Compass Ribbon */}
            <div className="w-full bg-white/95 backdrop-blur-md border border-slate-200 h-12 rounded-xl relative overflow-hidden flex flex-col justify-between items-center shadow-[0_8px_25px_rgba(0,0,0,0.02)] pt-1">
              ...
              <div className="w-full h-6 relative overflow-hidden flex justify-center">
                ...
                <div 
                  className="absolute left-1/2 -translate-x-1/2 bottom-0 w-2.5 h-2 bg-blue-600 z-10"
                ...
                >
                  {compassTicks.map(({ d, label, isCardinal }) => (
                    ...
                    >
                      ...
                      {/* Tick label */}
                      <span className={`text-[9px] font-mono mt-0.5 ${isCardinal ? 'text-slate-800 font-bold' : 'text-slate-400'}`}>
                        {label}
                      </span>
                    </div>
                  ))}
            ...
            <div className="mt-2 px-3 py-1 bg-white/80 border border-slate-200 rounded-full text-[9px] font-mono text-slate-500 shadow-sm">
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          {showTelemetryDashboard && (
            <div className="bg-white/95 backdrop-blur-md border border-slate-200 p-3 rounded-xl flex items-center gap-3.5 shadow-[0_8px_25px_rgba(0,0,0,0.02)]">
              <div className="text-right">
                <span className="text-[9px] text-slate-400 font-mono tracking-widest uppercase block leading-none mb-1">
                  LIPO BATTERY
                </span>
          ...
          <div className="flex flex-col items-end gap-1.5">
            <div className="bg-blue-50 border border-blue-100 px-3 py-1 rounded-full text-[9px] font-mono font-bold text-blue-600 shadow-sm uppercase tracking-wider">
              CAM VIEW: {flightCameraView}
            </div>
            <div className="bg-white/95 backdrop-blur-md border border-slate-200 p-1 rounded-xl flex gap-1 shadow-[0_8px_25px_rgba(0,0,0,0.02)] text-[10px] font-bold uppercase tracking-wider">
        ...
        {warnings.length > 0 && (
          <div className="space-y-2.5 flex flex-col items-center">
            {warnings.map((warn) => (
              <div 
                key={warn} 
                className="bg-red-50 border-2 border-red-300 px-6 py-3 rounded-2xl flex items-center gap-3 shadow-[0_10px_30px_rgba(239,68,68,0.1)] animate-pulse"
              >
                <ShieldAlert className="w-6 h-6 text-red-600" />
                <span className="text-sm font-bold text-red-800 font-mono tracking-widest uppercase">
                  ALERT: {warn}
                </span>
              </div>
            ))}
      ...
      >
        <div className="flex items-stretch h-full">
          ...
          <button 
            ...
            className="w-8 bg-white/95 backdrop-blur-md border-l border-t border-b border-slate-200 hover:bg-slate-50 rounded-l-xl flex flex-col items-center justify-center text-slate-500 hover:text-slate-800 transition-all shadow-md"
          >
            <span 
          ...
          {/* Main Panel Content */}
          <div className="flex-1 bg-white/95 backdrop-blur-md border border-slate-200 p-4 rounded-r-xl flex flex-col justify-between shadow-md overflow-hidden">
            <div className="space-y-4 flex flex-col h-full overflow-hidden">
              ...
              <div className="flex-1 flex flex-col min-h-0">
                <span className="text-[8px] text-slate-400 font-mono tracking-wider uppercase mb-1 flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-pulse"></span>
                  $PXTWIN Serial Stream (10Hz)
                </span>
                <div 
                  ref={terminalRef}
                  className="flex-1 bg-slate-55 border border-slate-200 p-2.5 rounded-lg font-mono text-[9px] text-slate-700 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-200 leading-normal"
                >
                  {appLinkStatus === 'connected' ? (
                    appTelemetryPackets.length > 0 ? (
                      [...appTelemetryPackets].reverse().map((pkt, idx) => (
                        <div key={idx} className="hover:bg-slate-100 truncate select-text py-0.5 border-b border-slate-100">
                          {pkt}
                        </div>
                      ))
                    ) : (
                      <div className="text-slate-400 animate-pulse italic">Waiting for telemetry frames...</div>
                    )
                  ) : appLinkStatus === 'connecting' ? (
                    <div className="text-amber-600 animate-pulse">Negotiating link layer handshake...</div>
                  ) : (
                    <div className="text-slate-400 italic font-light leading-relaxed">
              ...
              <div className="space-y-2 pt-2 border-t border-slate-100">
                <span className="text-[8px] text-slate-405 font-mono tracking-wider uppercase block">
                  ...
                  {[
                    ...
                    return (
                      <div key={sensor.key} className="flex justify-between items-center text-[10px] font-mono">
                        <span className="text-slate-500">{sensor.name}</span>
      ...
      <div className="w-full flex justify-between items-end pointer-events-auto gap-4">
        ...
        <div className="bg-white/95 backdrop-blur-md border border-slate-200 p-4 rounded-2xl w-[280px] shadow-[0_8px_25px_rgba(0,0,0,0.02)] flex flex-col gap-3">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            <span className="text-[10px] text-slate-400 font-mono tracking-widest uppercase">Flight Settings</span>
            <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded ${
              telemetry.isArmed ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-slate-100 text-slate-400 border border-slate-200'
            }`}>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs text-slate-600 font-medium">Altitude Hold (Auto-Level)</span>
              <button 
                ...
                className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                  telemetry.flightMode === 'althold' ? 'bg-blue-600' : 'bg-slate-200'
                }`}
              >
            ...
            <div className="flex justify-between text-[11px] font-mono pt-1 text-slate-500">
              ...
              <span className="text-blue-600 font-bold">{getFlightModeLabel(telemetry.flightMode)}</span>
            </div>
          </div>

          {/* HUD settings toggle buttons */}
          <div className="space-y-2 border-t border-slate-100 pt-2 text-[11px] text-slate-500 font-mono font-bold">
            <div className="flex justify-between items-center">
              <span>TELEMETRY HUD (T)</span>
              <button 
                onClick={toggleTelemetryDashboard}
                className={`w-9 h-5 rounded-full p-0.5 transition-colors duration-200 focus:outline-none ${showTelemetryDashboard ? 'bg-blue-600' : 'bg-slate-200'}`}
              >
            ...
            <div className="flex justify-between items-center">
              ...
              <button 
                ...
                className={`w-9 h-5 rounded-full p-0.5 transition-colors duration-200 focus:outline-none ${showControlsOverlay ? 'bg-blue-600' : 'bg-slate-200'}`}
              >
            ...
            <div className="flex justify-between items-center">
              ...
              <button 
                ...
                className={`w-9 h-5 rounded-full p-0.5 transition-colors duration-200 focus:outline-none ${showChecklist ? 'bg-blue-600' : 'bg-slate-200'}`}
              >
            ...
            <div className="flex justify-between items-center">
              ...
              <button 
                ...
                className={`w-9 h-5 rounded-full p-0.5 transition-colors duration-200 focus:outline-none ${isSpawnDebugMode ? 'bg-blue-600' : 'bg-slate-200'}`}
              >
          ...
          <div className="flex flex-col gap-2 mt-1 border-t border-slate-100 pt-2.5">
            <button
              ...
              className={`py-2 rounded-lg text-[10px] font-bold uppercase tracking-wider transition-all border w-full ${
                telemetry.isArmed
                  ? 'bg-red-50 border-red-450 text-red-600 hover:bg-red-650 hover:text-white shadow-sm'
                  : 'bg-emerald-50 border-emerald-500 text-emerald-700 hover:bg-emerald-600 hover:text-white shadow-sm disabled:opacity-40 disabled:hover:bg-slate-50 disabled:hover:text-slate-400 disabled:border-slate-200 disabled:shadow-none'
              }`}
            >
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className="py-1.5 bg-slate-50 hover:bg-slate-100 disabled:opacity-40 disabled:hover:bg-slate-50 border border-slate-200 text-[10px] font-bold uppercase tracking-wider rounded-lg text-slate-700 transition shadow-sm"
              >
                Calibrate
              </button>
              <button
                onClick={onReset}
                className="py-1.5 bg-red-50 hover:bg-red-100 border border-red-200 text-[10px] font-bold uppercase tracking-wider rounded-lg text-red-600 transition flex items-center justify-center gap-1.5 shadow-sm"
              >
        ...
        {showControlsOverlay ? (
          <div className="bg-white/95 backdrop-blur-md border border-slate-200 p-4 rounded-2xl shadow-[0_8px_25px_rgba(0,0,0,0.02)] flex gap-6 items-center">
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="w-24 h-24 bg-slate-50 rounded-xl relative border border-slate-200 flex items-center justify-center">
                {/* Grid axes */}
                <div className="absolute w-px h-full bg-slate-200/50"></div>
                <div className="absolute h-px w-full bg-slate-200/50"></div>
                ...
                >
                  <span className="absolute left-6 text-[10px] font-mono text-blue-650 font-bold whitespace-nowrap">
                    {Math.round(stickState.throttle * 100)}%
                  </span>
                </div>
              </div>
              <div className="flex justify-between w-full text-[9px] font-mono text-slate-550 mt-1">
                <span>YAW</span>
            ...
            <div className="flex flex-col items-center">
              <span className="text-[9px] text-slate-550 font-mono tracking-wider uppercase mb-1.5">Right Stick (Arrows)</span>
              <div className="w-24 h-24 bg-slate-50 rounded-xl relative border border-slate-200 flex items-center justify-center">
                {/* Grid axes */}
                <div className="absolute w-px h-full bg-slate-200/50"></div>
                <div className="absolute h-px w-full bg-slate-200/50"></div>
        ...
        {showTelemetryDashboard ? (
          <div className="bg-white/95 backdrop-blur-md border border-slate-200 p-4 rounded-2xl w-[280px] shadow-[0_8px_25px_rgba(0,0,0,0.02)] flex flex-col gap-3">
            <div className="flex items-center justify-between border-b border-slate-105 pb-2">
              <span className="text-[10px] text-slate-400 font-mono tracking-widest uppercase">Motor Diagnostics</span>
            ...
            <div className="grid grid-cols-2 gap-2 text-[10px] font-mono">
              {telemetry.motorRPMs.map((rpm, idx) => {
                ...
                return (
                  <div key={idx} className="bg-slate-50 border border-slate-200 p-1.5 rounded-lg flex flex-col">
                    <span className="text-slate-400 text-[8px]">{cornerNames[idx]}</span>
                    <span className={`font-bold mt-0.5 ${rpm > 0 ? 'text-orange-600' : 'text-slate-400'}`}>
            ...
            <div className="flex justify-between items-center text-[10px] font-mono pt-1 text-slate-500">
              <span>ALTITUDE:</span>
              <span className="text-slate-800 font-bold">{telemetry.altitude.toFixed(2)}m</span>
            </div>
          </div>
      ...
      {isInitChecking && !droneInitFailed && modelLoadStatus !== 'failed' && (
        <div className="absolute inset-0 bg-slate-900/25 backdrop-blur-md z-30 pointer-events-auto flex items-center justify-center font-mono">
          <div className="bg-white border border-slate-200 p-8 rounded-2xl w-full max-w-sm shadow-xl flex flex-col gap-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-500">1. Model Loaded</span>
                {isModelLoaded ? (
              ...
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-500">2. Physics Initialized</span>
                {!isModelLoaded ? (
                  <Circle className="w-4 h-4 text-slate-200" />
                ) : isPhysicsInitialized ? (
                  <CheckCircle className="w-4 h-4 text-emerald-600" />
                ) : (
                  <div className="w-4 h-4 border-2 border-blue-650 border-t-transparent rounded-full animate-spin"></div>
                )}
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-500">3. Drone Spawned</span>
                {!isPhysicsInitialized ? (
                  <Circle className="w-4 h-4 text-slate-200" />
                ) : isDroneSpawned ? (
                  <CheckCircle className="w-4 h-4 text-emerald-600" />
                ) : (
                  <div className="w-4 h-4 border-2 border-blue-655 border-t-transparent rounded-full animate-spin"></div>
                )}
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-500">4. Camera Auto-Focus</span>
                {!isDroneSpawned ? (
                  <Circle className="w-4 h-4 text-slate-200" />
                ) : isCameraLocked ? (
                  <CheckCircle className="w-4 h-4 text-emerald-600" />
                ) : (
                  <div className="w-4 h-4 border-2 border-blue-660 border-t-transparent rounded-full animate-spin"></div>
                )}
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-slate-500">5. Telemetry Ready</span>
                {!isCameraLocked ? (
                  <Circle className="w-4 h-4 text-slate-200" />
                ) : isTelemetryReady ? (
                  <CheckCircle className="w-4 h-4 text-emerald-600" />
                ) : (
                  <div className="w-4 h-4 border-2 border-blue-665 border-t-transparent rounded-full animate-spin"></div>
                )}
              </div>
            </div>
            <div className="text-[10px] text-slate-400 border-t border-slate-100 pt-3 text-center uppercase tracking-wider">
      ...
      {droneInitFailed && modelLoadStatus !== 'failed' && (
        <div className="absolute inset-0 bg-slate-900/35 backdrop-blur-md z-40 pointer-events-auto flex items-center justify-center font-mono">
          <div className="bg-white border border-slate-200 p-8 rounded-2xl w-full max-w-sm shadow-xl flex flex-col items-center text-center gap-5">
            ...
            <div>
              <h2 className="text-md font-bold text-red-600 uppercase tracking-widest">
            ...
            <div className="bg-slate-50 border border-slate-200 p-3 rounded-lg text-left text-[9px] text-slate-600 w-full font-mono">
              <span className="text-red-600 font-bold block mb-1">Fail Safe Log:</span>
            ...
            <button
              ...
              className="px-6 py-2 bg-red-50 hover:bg-red-100 border border-red-200 text-xs font-bold text-red-600 rounded-lg transition shadow-sm w-full"
            >
      ...
      {isSpawnDebugMode && droneSpawnDiagnostics && (
        <div className="absolute bottom-40 left-4 bg-white/95 border border-slate-200 p-4 rounded-xl w-72 shadow-lg pointer-events-auto z-20 font-mono text-[9px] text-slate-700 flex flex-col gap-2">
          <div className="flex items-center gap-1.5 border-b border-slate-100 pb-1.5 justify-between">
            <div className="flex items-center gap-1">
              <span className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-ping"></span>
              <span className="font-bold text-slate-900 uppercase tracking-wider">Spawn Diagnostics</span>
            </div>
            <span className="text-[8px] bg-blue-50 border border-blue-100 px-1 rounded text-blue-600 font-bold">DEBUG</span>
          </div>
          <div className="space-y-1.5">
            ...
            <div className="flex justify-between">
              <span className="text-slate-400">Physics Position:</span>
              <span className="text-slate-800">
                [{droneSpawnDiagnostics.physicsPos.map((v: number) => v.toFixed(3)).join(', ')}]
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Model Center Offset:</span>
              <span className="text-slate-800">
                [{droneSpawnDiagnostics.modelPos.map((v: number) => v.toFixed(3)).join(', ')}]
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Bounding Box Min Y:</span>
              <span className="text-slate-800">{droneSpawnDiagnostics.boxMinY.toFixed(4)}m</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Ground Level (Pad):</span>
              <span className="text-slate-800">0.005m</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Physics Floor Limit:</span>
              <span className="text-slate-800">{droneSpawnDiagnostics.groundHeight.toFixed(3)}m</span>
            </div>
            {useDroneStore.getState().modelDiagnostics && (
              <div className="border-t border-slate-100 pt-1.5 mt-1 text-[8px] text-slate-400 space-y-1">
